 ...
 *           - UI: consistent save buttons, form-hints, page-headers, Move→Apply
 *   v4.1.2  - Wake-up fix:
...
// Datalog format options (битови флагове за компактност)
...
struct DatalogConfig {
    ...
    // Format options (използваме reserved байтове)
...
String getNetworkDisplay() {
    ...
    return "📡 AP: " + String(strlen(config.network.apSSID) > 0 ? config.network.apSSID : config.deviceName);
}
